nonRDFaLite = empty######################################################################## RELAX NG Schema
for HTML 5: RDFa 1.1 and RDFa Lite 1.1#######################################################################
...
common.attrs.rdfa.property =
	...
	(
		list {
		(common.data.rdfa.term | common.data.rdfa.curie | common.data.uri.absolute) +
	} | string ""
)
}
common.attrs.rdfa.resource =
	attribute resource {
	(common.data.rdfa.safecurie | common.data.rdfa.curie | common.data.uri)
}
common.attrs.rdfa.typeof =
	attribute typeof {
(
...
common.attrs.rdfa.vocab =
	attribute vocab {
	(common.data.uri.absolute | string "")
}
common.attrs.rdfa.about =
	attribute about {
	(common.data.rdfa.safecurie | common.data.rdfa.curie | common.data.uri)
...
link.rdfa.elem.metadata =
	element link {
	link.inner & link.rdfa.attrs.metadata
}
link.rdfa.elem.phrasing =
	element link {
	link.inner & link.rdfa.attrs.phrasing
}
link.rdfa.attrs.metadata =
	(common.attrs.basic & common.attrs.i18n & common.attrs.present & common.attrs.other & ((common.attrs.rdfa.property & link.attrs.rel ? ) |
			(common.attrs.rdfa.property ?
			& link.attrs.rel
		)
	) & common.attrs.rdfa.about ?
...
& link.attrs.sizes ?
& link.attrs.as ? #link.attrs.title included in common.attrs & embedded.content.attrs.crossorigin ?
...
& shared - hyperlink.attrs.media ?
& shared - hyperlink.attrs.type ?
& link.attrs.sizes ? #link.attrs.title included in common.attrs & embedded.content.attrs.crossorigin ?
& (common.attrs.aria.role.link | common.attrs.aria.role.presentation | common.attrs.aria.role.menuitem) ?
)
common.elem.metadata |= link.rdfa.elem.metadata
common.elem.phrasing |= link.rdfa.elem.phrasing

## < meta property = foo content = bar > in head("name"
attribute optional)
meta.property.elem =
	element meta {
	meta.inner & meta.property.attrs
}
meta.property.attrs =
	(common.attrs.basic & common.attrs.i18n & common.attrs.present & common.attrs.other & meta.name.attrs.name ?
	& common.attrs.rdfa.property & meta.name.attrs.content & (common.attrs.aria.role.presentation | common.attrs.aria.role.menuitem) ?
)
common.elem.metadata |= meta.property.elem

## < meta property = foo content = bar > in body("name"
attribute disallowed)
meta.property - no - name.elem =
	element meta {
	meta.inner & meta.property - no - name.attrs
}
meta.property - no - name.attrs =
	(common.attrs.basic & common.attrs.i18n & common.attrs.present & common.attrs.other & common.attrs.rdfa.property & meta.name.attrs.content & (common.attrs.aria.role.presentation | common.attrs.aria.role.menuitem) ? )
